class User < ApplicationRecord
  ...
  def create_reset_digest
    self.reset_token = User.new_token     # トークンの生成し、reset_tokenに代入する
    update_columns(reset_digest: User.digest(reset_token),    # 生成したトークンをハッシュ化してreset_digestに格納する
  ...
  def send_password_reset_email
    UserMailer.password_reset(self).deliver_now     # password_resetメソッドを呼び出しているオブジェクト自身を引数に与える。→メールの送信
  end
